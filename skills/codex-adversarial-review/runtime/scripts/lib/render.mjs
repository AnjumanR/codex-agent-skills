...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    const actions = [`${STATUS_COMMAND_PREFIX} ${job.id}`];
    if (job.status === "queued" || job.status === "running") {
      actions.push(`${CANCEL_COMMAND_PREFIX} ${job.id}`);
    }
    lines.push(
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if ((job.status === "queued" || job.status === "running") && options.showCancelHint) {
    lines.push(`  Cancel: ${CANCEL_COMMAND_PREFIX} ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && options.showResultHint) {
    lines.push(`  Result: ${RESULT_COMMAND_PREFIX} ${job.id}`);
  }
  if (job.status !== "queued" && job.status !== "running" && job.jobClass === "task" && job.write && options.showReviewHint) {
    lines.push(`  Review changes: ${REVIEW_COMMAND}`);
    lines.push(`  Stricter review: ${ADVERSARIAL_REVIEW_COMMAND}`);
  }
...
export function renderCancelReport(job) {
  ...
  lines.push(`- Check \`${STATUS_COMMAND_PREFIX}\` for the updated queue.`);

  return `${lines.join("\n").trimEnd()}\n`;
}
const STATUS_COMMAND_PREFIX = "node scripts/codex-companion.mjs status";
const CANCEL_COMMAND_PREFIX = "node scripts/codex-companion.mjs cancel";
const RESULT_COMMAND_PREFIX = "node scripts/codex-companion.mjs result";
const REVIEW_COMMAND = "node scripts/codex-companion.mjs review --wait";
const ADVERSARIAL_REVIEW_COMMAND = "node scripts/codex-companion.mjs adversarial-review --wait";
