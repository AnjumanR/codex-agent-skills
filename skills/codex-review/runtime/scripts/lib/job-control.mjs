...
const STATUS_COMMAND = "node scripts/codex-companion.mjs status";
const CANCEL_COMMAND = "node scripts/codex-companion.mjs cancel";
...
function matchJobReference(jobs, reference, predicate = () => true) {
  ...
  throw new Error(`No job found for "${reference}". Run \`${STATUS_COMMAND}\` to list known jobs.`);
}
...
export function buildSingleJobSnapshot(cwd, reference, options = {}) {
  ...
  if (!selected) {
    throw new Error(`No job found for "${reference}". Run \`${STATUS_COMMAND}\` to inspect known jobs.`);
  }
...
export function resolveResultJob(cwd, reference) {
  ...
  if (active) {
    throw new Error(`Job ${active.id} is still ${active.status}. Check \`${STATUS_COMMAND}\` and try again once it finishes.`);
  }

  if (reference) {
    throw new Error(`No finished job found for "${reference}". Run \`${STATUS_COMMAND}\` to inspect active jobs.`);
  }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) {
    throw new Error(`Multiple Codex jobs are active. Pass a job id to \`${CANCEL_COMMAND}\`.`);
  }
