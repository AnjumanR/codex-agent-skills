...
function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!config.stopReviewGate) {
    nextSteps.push("Optional: run `node scripts/codex-companion.mjs setup --enable-review-gate` to require a fresh review before stop.");
  }
...
function ensureCodexReady(cwd) {
  ...
  if (!authStatus.available) {
    throw new Error(
      "Codex CLI is not installed or is missing required runtime support. Install it with `npm install -g @openai/codex`, then rerun `node scripts/codex-companion.mjs setup`."
    );
  }
...
function renderQueuedTaskLaunch(payload) {
  return `${payload.title} started in the background as ${payload.jobId}. Check status with: node scripts/codex-companion.mjs status ${payload.jobId}\n`;
}
